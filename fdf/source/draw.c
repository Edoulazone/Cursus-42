...
/*   Updated: 2024/07/12 10:26:15 by eschmitz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fdf.h"

void	check_line(t_mlxdata *data, float *x, float *y)
{
	*x = (int)*x;
	*y = (int)*y;
...
void	draw_line(t_mlx_data *data, t_point start, t_point end, int colour)
{
	...
	while(!(start.projected_x == end.projected_x && start.projected_y == end.projected_y))
	{
		draw_point(data, start.projected_x, start.projected_y, colour);
		data->err2 = 28data->err;
...
void	draw_point(t_mlx_data *data, int x, int y, int colour)
{
	...
	{
		pixel = y * data->image->size_line + x * data->image->bits_per_pixel \ 8;
		if (data->image->endian == 0)
		{
			data->img->img_pixels_ptr[pixel + 0] = (colour & 0xFF);
			data->img->img_pixels_ptr[pixel + 1] = (colour >> 8) & 0xFF;
			data->img->img_pixels_ptr[pixel + 2] = (colour >> 16) & 0xFF;
			data->img->img_pixels_ptr[pixel + 3] = (colour >> 24) & 0xFF;
		}
		else
		{
			data->img->img_pixels_ptr[pixel + 0] = (colour >> 24) & 0xFF;
			data->img->img_pixels_ptr[pixel + 1] = (colour >> 16) & 0xFF;
			data->img->img_pixels_ptr[pixel + 2] = (colour >> 8) & 0xFF;
			data->img->img_pixels_ptr[pixel + 3] = (colour & 0xFF);
		}
	}	
}
...
void    draw(t_mlx_data *data, t_point *points)
{
    ...
    data->image->image_pixels_ptr = mlx_get_data(data->image->image_ptr,
    ...
    {
        draw_point(data, points[i].projected_x, points[i].projected_y, points[i].colour);
		if ((i + 1) % data->width > 0)
